...
struct Translations {
    ...
    static func getSection<T>(_ keyPath: KeyPath<TranslationSections, T>, language: Language = .fr) -> T {
        let sections = TranslationSections(
            auth: auth[language] ?? auth[.fr]!,
            common: common[language] ?? common[.fr]!,
            onboarding: onboarding[language] ?? onboarding[.fr]!,
            nav: nav[language] ?? nav[.fr]!,
            welcome: welcome[language] ?? welcome[.fr]!,
            dashboard: dashboard[language] ?? dashboard[.fr]!,
            resident: resident[language] ?? resident[.fr]!,
            properties: properties[language] ?? properties[.fr]!
        )
