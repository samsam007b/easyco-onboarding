...
export async function saveOnboardingData(userId: string, data: OnboardingData, userType: string) {
  ...
  try {
    ...
    if (data.hasGuarantor !== undefined) profileData.guarantor_available = data.hasGuarantor
    ...
    if (data.hasProperty !== undefined) profileData.has_property = data.hasProperty === 'yes' || data.hasProperty === true
