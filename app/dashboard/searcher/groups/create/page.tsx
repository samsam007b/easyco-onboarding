...
export default function CreateGroupPage() {
  ...
  const handleCreateGroup = async () => {
    ...
    if (maxMembers < 2 || maxMembers > 10) {
      toast.error(t?.messages?.sizeError?.[language] || 'La taille du groupe doit être entre 2 et 10 membres');
      return;
    }
    ...
    try {
      ...
      toast.error(error.message || t?.messages?.error?.[language] || 'Échec de la création du groupe');
    } finally {
      setIsCreating(false);
    }
  };
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-xl font-bold text-gray-900">{t?.basicInfo?.title?.[language] || 'Informations de base'}</h2>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder={t?.basicInfo?.descriptionPlaceholder?.[language] || 'Parlez de votre groupe, vos intérêts, votre style de vie...'}
                      rows={4}
                      maxLength={500}
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-3">
                      {t?.basicInfo?.maxMembers?.[language] || 'Nombre maximum de membres'}: <span className="text-[#FFA040] text-lg font-bold">{maxMembers}</span>
                    </label>
                    <input
                      type="range"
                      min="2"
              ...
              <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-xl font-bold text-gray-900">{t?.settings?.title?.[language] || 'Paramètres du groupe'}</h2>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100">
                    <div className="flex-1">
                      <div className="font-semibold text-gray-900 flex items-center gap-2">
                        {t?.settings?.approval?.title?.[language] || 'Nécessite une approbation'}
                      </div>
                      <div className="text-sm text-gray-600 mt-1">
                        {t?.settings?.approval?.description?.[language] || 'Examiner les demandes avant que les membres ne rejoignent'}
                      </div>
                    </div>
                    <button
              ...
              {/* Group Preferences */}
              <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                <h2 className="text-xl font-bold text-gray-900 mb-2">{t?.preferences?.title?.[language] || 'Préférences de recherche'}</h2>
                ...
                >
                  {isCreating ? (
                    <>
                      ...
                      {t?.actions?.creating?.[language] || 'Création en cours...'}
                    </>
                  ) : (
                    <>
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full p-8 shadow-2xl animate-scaleIn">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600">
                {t?.successModal?.subtitle?.[language]?.replace('{name}', createdGroup.name) || `Votre groupe "${createdGroup.name}" est maintenant prêt`}
              </p>
            </div>
            ...
            <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
              <div className="flex gap-3">
                ...
                <div className="text-sm text-gray-700">
                  ...
                  <p className="text-gray-600">
                    {t?.successModal?.tip?.content?.[language] || 'Invitez au moins 2-3 personnes pour commencer. Plus votre groupe est complet, plus vous aurez de chances de trouver rapidement !'}
                  </p>
                </div>
              </div>
            </div>
