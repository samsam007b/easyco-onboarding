...
import type { RefuseInvitationRequest, RefuseInvitationResponse } from '@/types/invitation.types';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Parse request body
    const body: RefuseInvitationRequest = await request.json();
    ...
    const { data, error } = await supabase.rpc('refuse_property_invitation', {
    ...
    const result = data as RefuseInvitationResponse;
    ...
    console.error('Error in refuse invitation API:', error);
    return NextResponse.json(
