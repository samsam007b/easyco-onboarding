...
export default function EnhanceReviewPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!user) {
        toast.error(t('profileEnhance.review.toast.loginRequired'));
        return;
      }
      ...
      if (result.success) {
        ...
        toast.success(t('profileEnhance.review.toast.success'));

        setTimeout(() => {
          router.push(`/dashboard/${userType}`);
        }, 1000);
      } else {
        throw new Error(t('profileEnhance.review.toast.saveFailed'));
      }
    } catch (error: any) {
      // FIXME: Use logger.error('Error saving profile:', error);
      toast.error(t('profileEnhance.review.toast.error') + ': ' + error.message);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <EnhanceProfileLayout
      ...
      backLabel={t('profileEnhance.common.back')}
      progress={{
        current: 6,
        total: 6,
        label: `${t('profileEnhance.review.progress.step')} 6 ${t('profileEnhance.review.progress.of')} 6`,
        stepName: t('profileEnhance.review.progress.stepName'),
      }}
      isLoading={isLoading}
      loadingText={t('profileEnhance.review.loading')}
    >
      <EnhanceProfileHeading
        role="searcher"
        title={t('profileEnhance.review.title')}
        description={t('profileEnhance.review.description')}
        ...
        <EnhanceProfileInfoBox role="searcher" icon="⚠️">
          <div className="flex items-start gap-3">
            ...
            <p className="text-gray-700">
              {t('profileEnhance.review.noData')}
            </p>
          </div>
        </EnhanceProfileInfoBox>
      ) : (
          ...
          {(data.about?.bio || data.about?.aboutMe || data.about?.lookingFor) && (
            <EnhanceProfileSection title={t('profileEnhance.review.sections.about')}>
              <div className="bg-white p-6 superellipse-xl border border-gray-200 space-y-4">
                ...
                {data.about.aboutMe && (
                  <div>
                    <p className="text-sm font-semibold text-gray-700 mb-1">{t('profileEnhance.review.fields.aboutMe')}:</p>
                    <p className="text-gray-600">{data.about.aboutMe}</p>
                  </div>
                )}
                {data.about.lookingFor && (
                  <div>
                    <p className="text-sm font-semibold text-gray-700 mb-1">{t('profileEnhance.review.fields.lookingFor')}:</p>
          ...
          {(data.values?.coreValues?.length > 0 ||
            ...
            <EnhanceProfileSection title={t('profileEnhance.review.sections.values')}>
              <div className="bg-white p-6 superellipse-xl border border-gray-200 space-y-4">
                ...
                {data.values.importantQualities && data.values.importantQualities.length > 0 && (
                  <div>
                    <p className="text-sm font-semibold text-gray-700 mb-2">{t('profileEnhance.review.fields.importantQualities')}:</p>
                    <div className="flex flex-wrap gap-2">
                ...
                {data.values.dealBreakers && data.values.dealBreakers.length > 0 && (
                  <div>
                    <p className="text-sm font-semibold text-gray-700 mb-2">{t('profileEnhance.review.fields.dealBreakers')}:</p>
                    <div className="flex flex-wrap gap-2">
            ...
            <EnhanceProfileSection title={t('profileEnhance.review.sections.financial')}>
              <div className="bg-white p-6 superellipse-xl border border-gray-200 space-y-3">
                ...
                {data.financial.employmentType && (
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">{t('profileEnhance.review.fields.employmentType')}:</span>
                    <span className="text-sm font-medium text-gray-900">{data.financial.employmentType}</span>
                  </div>
                )}
                {data.financial.hasGuarantor !== undefined && (
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">{t('profileEnhance.review.fields.hasGuarantor')}:</span>
                    <span className="text-sm font-medium text-gray-900">{data.financial.hasGuarantor ? t('common.yes') : t('common.no')}</span>
                  </div>
                )}
              </div>
            </EnhanceProfileSection>
            ...
            <EnhanceProfileSection title={t('profileEnhance.review.sections.community')}>
              <div className="bg-white p-6 superellipse-xl border border-gray-200 space-y-3">
                ...
                {data.community.enjoySharedMeals !== undefined && (
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">{t('profileEnhance.review.fields.enjoySharedMeals')}:</span>
                    <span className="text-sm font-medium text-gray-900">{data.community.enjoySharedMeals ? t('common.yes') : t('common.no')}</span>
                  </div>
                )}
                {data.community.openToMeetups !== undefined && (
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">{t('profileEnhance.review.fields.openToMeetups')}:</span>
                    <span className="text-sm font-medium text-gray-900">{data.community.openToMeetups ? t('common.yes') : t('common.no')}</span>
                  </div>
                )}
              </div>
            </EnhanceProfileSection>
      ...
      <div className="space-y-3 mt-8">
        ...
        >
          {isSaving ? t('profileEnhance.review.buttons.saving') : t('profileEnhance.review.buttons.save')}
        </button>
        <button
        ...
        >
          {t('profileEnhance.review.buttons.skipForNow')}
        </button>
      </div>
    </EnhanceProfileLayout>
  );
