...
export const CardPile = memo(function CardPile({
  ...
  return (
    <div
      className={`flex flex-col items-center w-[80px] sm:w-[120px] md:w-[160px] ${
        isLike ? 'items-start' : 'items-end'
      }`}
    >
      {/* Pile header */}
      <div className={`mb-2 sm:mb-3 flex items-center gap-1 sm:gap-2 ${isLike ? '' : 'flex-row-reverse'}`}>
        <div className={`
          w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center
          ${isLike ? 'bg-green-100' : 'bg-red-100'}
        `}>
          {isLike ? (
            <Heart className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-green-600 fill-green-600" />
          ) : (
            <X className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-red-600" strokeWidth={3} />
          )}
        </div>
        <div className={`${isLike ? 'text-left' : 'text-right'} hidden sm:block`}>
      ...
      <div
        className={`
          relative w-[60px] h-[80px] sm:w-[80px] sm:h-[110px] md:w-[100px] md:h-[140px]
        `}
      >
      ...
      {cards.length > 0 && onUndo && (
        <motion.button
          ...
          onClick={onUndo}
          className={`
            mt-1.5 sm:mt-2 flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-medium
            transition-colors
            ${isLike
        ...
        >
          ...
          <span className="hidden sm:inline">{matching.cardPile?.undo || 'Annuler'}</span>
        </motion.button>
      )}
    </div>
  );
