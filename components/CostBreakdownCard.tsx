...
export default function CostBreakdownCard({
  ...
  const costBreakdown = getSection('property')?.costBreakdown;
      ...
      <CardContent className="p-0">
        ...
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-baseline justify-between">
            <span className="text-sm text-gray-600">{costBreakdown?.roomRent || 'Loyer chambre'}</span>
            <div className="text-right">
              <span className="text-2xl font-bold text-gray-900">€{selectedRoom.price}</span>
              <span className="text-sm text-gray-500">{costBreakdown?.perMonth || '/mois'}</span>
            </div>
          </div>
        ...
        <div className="border-b border-gray-200">
          ...
          >
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-gray-700">{costBreakdown?.charges || '+ Charges'}</span>
            ...
            <div className="px-4 pb-4 space-y-2 bg-gray-50">
              {costs.utilities.breakdown.electricity !== undefined && costs.utilities.breakdown.electricity > 0 && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">• {costBreakdown?.electricity || 'Électricité'}</span>
              ...
              {costs.utilities.breakdown.water !== undefined && costs.utilities.breakdown.water > 0 && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">• {costBreakdown?.water || 'Eau'}</span>
              ...
              {costs.utilities.breakdown.heating !== undefined && costs.utilities.breakdown.heating > 0 && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">• {costBreakdown?.heating || 'Chauffage'}</span>
              ...
              {costs.utilities.breakdown.internet !== undefined && costs.utilities.breakdown.internet > 0 && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">• {costBreakdown?.internet || 'Internet'}</span>
              ...
              {costs.utilities.breakdown.trash !== undefined && costs.utilities.breakdown.trash > 0 && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">• {costBreakdown?.trash || 'Ordures'}</span>
              ...
              {costs.utilities.breakdown.other !== undefined && costs.utilities.breakdown.other > 0 && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">• {costBreakdown?.other || 'Autres'}</span>
        ...
        {costs.shared_living.total > 0 && (
          <div className="border-b border-gray-200">
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-gray-700">{costBreakdown?.sharedLiving || '+ Vie commune'}</span>
                <Info className="w-4 h-4 text-gray-400" />
              </div>
              <div className="flex items-center gap-2">
            ...
            {showSharedLivingDetail && costs.shared_living.breakdown && (
              <div className="px-4 pb-4 space-y-2 bg-gray-50">
                {costs.shared_living.breakdown.cleaning_service !== undefined && costs.shared_living.breakdown.cleaning_service > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">• {costBreakdown?.cleaningService || 'Femme de ménage'}</span>
                ...
                {costs.shared_living.breakdown.wifi !== undefined && costs.shared_living.breakdown.wifi > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">• {costBreakdown?.wifiFiber || 'WiFi fibre'}</span>
                ...
                {costs.shared_living.breakdown.cleaning_supplies !== undefined && costs.shared_living.breakdown.cleaning_supplies > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">• {costBreakdown?.cleaningSupplies || 'Produits ménagers'}</span>
                ...
                {costs.shared_living.breakdown.shared_groceries !== undefined && costs.shared_living.breakdown.shared_groceries > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">• {costBreakdown?.sharedGroceries || 'Courses communes'}</span>
                ...
                {costs.shared_living.breakdown.maintenance !== undefined && costs.shared_living.breakdown.maintenance > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">• {costBreakdown?.maintenance || 'Maintenance'}</span>
                ...
                {costs.shared_living.breakdown.insurance !== undefined && costs.shared_living.breakdown.insurance > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">• {costBreakdown?.homeInsurance || 'Assurance habitation'}</span>
        ...
        <div className="p-4 bg-gradient-to-r from-orange-50 to-yellow-50">
          <div className="flex items-center justify-between">
            <span className="text-base font-semibold text-gray-900">{costBreakdown?.totalMonthly || 'TOTAL MENSUEL'}</span>
            <div className="text-right">
              <div className="text-3xl font-bold text-orange-600">€{totalMonthly}</div>
              <div className="text-xs text-gray-600">{costBreakdown?.perMonthSmall || 'par mois'}</div>
            </div>
          </div>
        </div>
          ...
          <div className="p-4 bg-blue-50 border-t border-blue-100">
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-blue-900 font-medium">{costBreakdown?.deposit || 'Dépôt de garantie'}</span>
              </div>
              <span className="text-blue-900 font-semibold">€{costs.deposit}</span>
            </div>
            <p className="text-xs text-blue-700 mt-1">{costBreakdown?.depositInfo || 'À payer une seule fois (remboursable)'}</p>
          </div>
        )}
      </CardContent>
    </Card>
