...
type KPIVariant = 'primary' | 'secondary' | 'success' | 'warning' | 'danger' | 'info';

interface SearcherKPICardProps {
  title: string;
  value: string | number;
  icon: LucideIcon;
  variant?: KPIVariant;
...
const variantStyles: Record<KPIVariant, { gradient: string; bgGradient: string; textColor: string }> = {
  primary: {
    ...
    bgGradient: searcherGradientLight,
    textColor: '#C67A30',
  },
  secondary: {
    gradient: 'linear-gradient(135deg, #FFB85C 0%, #FFD080 100%)',
    bgGradient: 'linear-gradient(135deg, #FFFBEA 0%, #FFF8E0 100%)',
    textColor: '#D49040',
  },
  success: {
...
export function SearcherKPICard({
  title,
  value,
  icon: Icon,
  variant = 'primary',
...
interface SearcherKPIGridProps {
  children: React.ReactNode;
  columns?: 2 | 3 | 4 | 5;
  className?: string;
}

export function SearcherKPIGrid({ children, columns = 4, className }: SearcherKPIGridProps) {
  const gridCols = {
