...
function PropertyCard({
  ...
  // Determine if we should show rooms dropdown or use classic link behavior
  const hasRoomsData = rooms && rooms.length > 0;

  // For backward compatibility: if no rooms data, use clickable Link wrapper
  if (!hasRoomsData) {
    return (
      <>
        <Link
...
// If rooms data exists, render non-clickable card with dropdown
return (
  <>
    <div
      className="block group"
    ...
    >
      <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all overflow-hidden">
        ...
        <div className="p-4 sm:p-6">
          ...
          {/* Rooms Dropdown - Always visible in this branch */}
          <RoomsDropdown rooms={rooms} propertyId={property.id} className="mb-4" />
